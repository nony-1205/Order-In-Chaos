...
(function(){
const thomasparams={b:0.18};
window.thomasparams=thomasparams
const dt= 0.005;
...
function thomas(x,y,z){
    const dx= Math.sin(y) - thomasparams.b*x;
    const dy=Math.sin(z) - thomasparams.b*y;
    const dz = Math.sin(x) - thomasparams.b*z;
    return [dx,dy,dz];}



function rk4(x,y,z,dt)
    {const [k1x, k1y, k1z] = thomas(x,y,z);
     const [k2x, k2y, k2z] = thomas(x+k1x*dt/2, y+k1y*dt/2, z+k1z*dt/2);
     const [k3x, k3y, k3z] = thomas(x+k2x*dt/2, y+k2y*dt/2, z+k2z*dt/2);
     const [k4x, k4y, k4z] = thomas(x+k3x*dt, y+k3y*dt, z+k3z*dt);
...
function initThomas(){
    ...
    const colorLimegreen = new THREE.Color(0x32cd32);
    const colorTeal = new THREE.Color(0x008080);
    ...
    if (t<0.05){
        const factor = t/0.05;
        tempcolor.copy(colorBlack).lerp(colorTeal, factor);
    }
    else{
        const factor = (t-0.05)/0.95;
        tempcolor.copy(colorTeal).lerp(colorWhite, factor);
    }
...
function destroyThomas(){
    cancelAnimationFrame(animationID);
    if (geometry) geometry.dispose();
    if (renderer) renderer.dispose();}

window.initThomas = initThomas;
window.destroyThomas = destroyThomas;
window.resetThomas=resetThomas;
})();