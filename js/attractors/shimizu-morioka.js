...
(function(){
const shimizuParams={a: 0.75, b: 0.45};
window.shimizuParams=shimizuParams;
const dt= 0.005;
...
function shimizu(x,y,z){
    const dx= y;
    const dy=x*(1-z)-shimizuParams.a*y;
    const dz = x*x -shimizuParams.b*z;
    return [dx,dy,dz];}



function rk4(x,y,z,dt)
    {const [k1x, k1y, k1z] = shimizu(x,y,z);
     const [k2x, k2y, k2z] = shimizu(x+k1x*dt/2, y+k1y*dt/2, z+k1z*dt/2);
     const [k3x, k3y, k3z] = shimizu(x+k2x*dt/2, y+k2y*dt/2, z+k2z*dt/2);
     const [k4x, k4y, k4z] = shimizu(x+k3x*dt, y+k3y*dt, z+k3z*dt);
...
function animate(){
    if (count<maxpoints){
        for (let i=0; i<stepsperframe; i++){
            ...
            positions[count*3]=(cx+1)*scale;
            positions[count*3+1]=(cz-1)*scale;
        ...
        for (let i=0; i<stepsperframe; i++){
            [cx,cy,cz]=rk4(cx,cy,cz,dt);
            positions[(maxpoints-stepsperframe+i)*3]=(cx+1)*scale;
            positions[((maxpoints-stepsperframe+i)*3)+1]=(cz-1)*scale;
...
function initShimizu(){
    ...
    if (t<0.05){
        const factor = t/0.05;
        tempcolor.copy(colorBlack).lerp(colorYellow, factor);
    }
    else{
        const factor = (t-0.05)/0.95;
        tempcolor.copy(colorYellow).lerp(colorOrange, factor);
    }
...
function destroyShimizu(){
    cancelAnimationFrame(animationID);
    if (geometry) geometry.dispose();
    if (renderer) renderer.dispose();}

window.initShimizu = initShimizu;
window.destroyShimizu = destroyShimizu;
window.resetShimizu=resetShimizu
})()