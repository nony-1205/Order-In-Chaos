//
(function(){
const chenParams={a:35, b:3, c:28}
window.chenParams=chenParams
const dt= 0.005;
...
function chen(x,y,z){
    const dx= chenParams.a*(y-x);
    const dy=(chenParams.c-chenParams.a)*x-x*z+chenParams.c*y;
    const dz = x*y-chenParams.b*z;
    return [dx,dy,dz];}



function rk4(x,y,z,dt)
    {const [k1x, k1y, k1z] = chen(x,y,z);
     const [k2x, k2y, k2z] = chen(x+k1x*dt/2, y+k1y*dt/2, z+k1z*dt/2);
     const [k3x, k3y, k3z] = chen(x+k2x*dt/2, y+k2y*dt/2, z+k2z*dt/2);
     const [k4x, k4y, k4z] = chen(x+k3x*dt, y+k3y*dt, z+k3z*dt);
...
function animate(){
    if (count<maxpoints){
        for (let i=0; i<stepsperframe; i++){
            ...
            positions[count*3+1]=(cz-20)*scale;
        ...
        for (let i=0; i<stepsperframe; i++){
            ...
            positions[((maxpoints-stepsperframe+i)*3)+1]=(cz-20)*scale;
...
function initChen(){
    ...
    colors.fill(0);
    const colordeepteal= new THREE.Color(0x004d4d);
    const colorelectricblue = new THREE.Color(0x7df9ff);
    ...
    if (t<0.05){
        const factor = t/0.05;
        tempcolor.copy(colordeepteal).lerp(colorelectricblue, factor);
    }
    else{
        const factor = (t-0.05)/0.95;
        tempcolor.copy(colorelectricblue).lerp(colorWhite, factor);
    }
...
function destroyChen(){
    cancelAnimationFrame(animationID);
    if (geometry) geometry.dispose();
    if (renderer) renderer.dispose();}

window.initChen = initChen;
window.destroyChen = destroyChen;
window.resetChen=resetChen
})();