...
window.lorenzParams=lorenzParams
const dt= 0.005;
...
function animate(){
    if (count<maxpoints){
        for (let i=0; i<stepsperframe; i++){
            ...
            positions[count*3+1]=(cz-25)*scale;
        ...
        for (let i=0; i<stepsperframe; i++){
            ...
            positions[((maxpoints-stepsperframe+i)*3)+1]=(cz-25)*scale;
...
function initLorenz(){
    ...
    const lineMaterial = new THREE.LineBasicMaterial({vertexColors: true, opacity: 0.4, depthWrite: false, blending: THREE.AdditiveBlending});
    ...
    const colorNavy = new THREE.Color(0x000080);
    ...
    if (t<0.4){
        const factor = t/0.4;
        tempcolor.copy(colorBlack).lerp(colorNavy, factor);
    }
    else if (t<0.75){
        const factor = (t-0.4)/0.35;
        tempcolor.copy(colorNavy).lerp(colorCyan, factor);
    }
    else{
...
function destroyLorenz(){
    cancelAnimationFrame(animationID);
    if (geometry) geometry.dispose();
    if (renderer) renderer.dispose();}

window.initLorenz = initLorenz;
window.destroyLorenz = destroyLorenz;
window.resetLorenz=resetLorenz
